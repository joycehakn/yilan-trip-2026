...
const rows=[
 ...
 {t:'09:30–11:30',common:true,title:'🦌 上午三選一',desc:'每個選項皆附各自的停車地點',places:[['斑比山丘','斑比山丘','斑比山丘 停車場'],['宜農牧場','宜農牧場','宜農牧場 停車場'],['梅花湖','梅花湖風景區','梅花湖 公有停車場']]},
...
function cell(r,group,main){if(!r.common)return r[group.toLowerCase()]||'—';if(!main)return '<span class="同行">同行 ←</span>';let maps='';if(r.places)maps=r.places.map(([n,q,p])=>'<div class="links">'+link('📍 '+n,q)+' '+link('🅿️ '+n+'停車',p,group==='A'?'apple':'google')+'</div>').join('');else maps='<div class="links">'+both(r.place,r.parking,group)+'</div>';return `<b>${r.title}</b><br>${r.desc}${maps}`}
